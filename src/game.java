...
public class game {
	...
	public static void generateSequence(int seqLength, int[] red, int[] blue, int[] green, int[] yellow)
		...
		try {
			...
			// System.out.println("colourChoice : " + colourChoice); //For Testing

			Underlight[] underlights = new Underlight[] { Underlight.FRONT_LEFT, Underlight.BACK_LEFT,
					Underlight.FRONT_RIGHT, Underlight.BACK_RIGHT };
	...
	private static final int POST_SEQUENCE_DELAY = 500;
	...
	public static void main(String[] args) {
		...
		/// Initialisation of swiftBot object
		...
		System.out.println("The Simon Game - SwiftBot Edition");
		while (!gameOver) {
//			System.out.println("Round: " + round + ", Score: " + score);
			...
			for (int i = 0; i < sequenceLength; i++) {
				...
				if (displayedColor != userSelectedColor) {
					lightIndicators.SequenceDecision(swiftBot, RED);
					if (score >= 5)
						victoryDive.Dive(swiftBot);
			...
			if (sequenceCorrect) {
				...
				lightIndicators.SequenceDecision(swiftBot, GREEN);
				try {
					Thread.sleep(500);
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
			...
			if (score > 0 && score % 5 == 0) {
				...
				while (confirmation.length() == 0) {
					...
					System.out
							.println("Do you want to quit or continue? Type 'continue' to proceed or 'quit' to quit:");
